# RouterOS 7.14+ Back-to-Home installer
# Import once, then call $installBTH with named parameters.
...
:global installBTH do={
    ...
    :if ($resetAll = "yes") do={
        ...
        /ip/firewall/filter/remove [find where comment=("installBTH:firewall:" . $u)]
        /ip/firewall/address-list/remove [find where comment~("installBTH:access:" . $u . ":")]
        /ip/address/remove [find where comment=("installBTH:address:" . $u)]
    }

    /ip/address/add interface=$bthIf address=$a comment=("installBTH:address:" . $u)
    ...
    /ip/firewall/filter/add chain=input action=accept protocol=$proto dst-port=$portList in-interface=$bthIf src-address=$clientCIDR place-before=0 comment=("installBTH:firewall:" . $u)
    :foreach token in=[:toarray $accessList] do={
        /ip/firewall/address-list/remove [find where list=$token and address=$clientCIDR and comment~"installBTH:"]
        /ip/firewall/address-list/add list=$token address=$clientCIDR comment=("installBTH:access:" . $u . ":" . $token)
    }
    ...
    /file/add name=$sumName contents=($digest . "  " . $fileName . "\n")
